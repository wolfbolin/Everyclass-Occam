...
def fetch_schedule_card(config, version, task_name, task_word, url_index, semester, active_list):
    """
...
def pull_table_page(config, version, task_key, url_index, headers, obj_data, ):
    ...
    if http_result is None:
        raise Util.NetworkError("获取【%s】对象<%s>时，网络请求失败" % (task_key[0], extra_data["name"]))
...
def parse_list_page(config, version, task_key, http_result, obj_data):
    ...
    print(json.dumps(lesson_list, ensure_ascii=False))
