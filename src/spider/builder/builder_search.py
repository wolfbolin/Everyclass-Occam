...
def build_search_all():
    ...
    util.print_t('Step2:正在处理教师搜索文档数据')
...
def build_search_semester(semester):
    ...
    util.print_i('Step1.1:正在删除旧的数据')
    mongo_conn = database.mongo_connect()
    sql_count = database.delete_semester_document(mongo_conn, 'student', semester)
    rowcount += sql_count
    ...
    util.print_i('Step2.1:正在删除旧的数据')
    mongo_conn = database.mongo_connect()
    sql_count = database.delete_semester_document(mongo_conn, 'teacher', semester)
    rowcount += sql_count
